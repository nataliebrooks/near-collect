...
impl Contract {
    ...
    #[payable]
    pub fn create_order(&mut self, nft_contract_id: AccountId, token_id: String) {
        //assert that the user has attached exactly 1 yoctoNEAR (for security reasons)
        assert_one_yocto();

        let requester_and_token_id = format!("{}{}{}", nft_contract_id, DELIMETER, token_id);

        let order = Order {
            // requester_id: receiver_id, // How can I grab caller from context?
            // owner_id: How can I grab the owner of the token
            // assignee: owner_id
            nft_contract_id: nft_contract_id, // Idk if I need this...
            token_id: token_id
        ...
        assert!(
            self.orders.insert(&requester_and_token_id, &order).is_none(),
            "Order already exists"
        );
        // insert into more organized sets
        self.by_assignee.insert(&order.assignee, &order);
        self.by_requester.insert(&order.requester, &order);
    }

    #[payable]
    pub fn accept_order(&mut self, requester: AccountId, token_id: String) -> String {

        let requester_and_token_id = format!("{}{}{}", nft_contract_id, DELIMETER, token_id);
        
        let mut order = self.orders.get(&requester_and_token_id).expect("No order for token from requester");
    ...
    #[payable]
    pub fn execute_order(&mut self, requester: AccountId, token_id: String) {
    ...
    #[payable]
    pub fn complete_order(&mut self, requester: AccountId, token_id: String) {
